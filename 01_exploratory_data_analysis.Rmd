...
```{r profiling-data-in-r, echo=FALSE, fig.cap="데이터셋 상태", out.extra=""}
...
```{r frequency-analysis-r, fig.height=3, fig.width=5, fig.cap=c('빈도 분석 1','빈도 분석 2'), out.extra = ""}
...
```{r univariate-analysis, echo=FALSE, fig.cap="순서와 아름다움", out.extra = ''}
...
```{r matrix-movie, out.width="150px", echo=FALSE, fig.cap="데이터의 매트릭스", out.extra = ""}
...
```{r, profiling-numerical-variable-with-histograms, fig.cap="수치형 데이터 프로파일링", out.extra = ''}
...
```{r anscombe-data, message=FALSE, tidy=FALSE, fig.cap="앤스콤 셋", out.extra = ''}
...
```{r mic-non-linear-relationship, message=FALSE, fig.width=4, fig.height=3, fig.cap="완벽한 관계", out.extra = ''}
...
```{r noisy-relationship, fig.width=4, fig.height=3, fig.cap="노이즈 추가", out.extra = ''}
...
```{r measuring-non-linearity, message=FALSE, fig.width=8, fig.height=3, tidy=FALSE, fig.cap="관계 비교", out.extra = ''}
# 데이터 예시 생성
...
```{r anscombe-set-information, fig.cap="MIC 통계량", out.extra = ''}
# 모든 쌍에 대한 MIC 계산
...
```{r monotonic-non-monotonic-function, fig.height=3.5, fig.width=8, tidy=FALSE, fig.cap="함수의 단조성", out.extra = ''}
# 샘플 데이터 생성 (시계열 시뮬레이션)
...
```{r periodical-function, fig.height=3.5, fig.width=8, tidy=FALSE, fig.cap="함수의 주기성", out.extra = '', echo=FALSE}
# 비주기적 함수에 대한 데이터 생성
...
```{r correlation-time-series, fig.height=3, fig.width=5, tidy=FALSE, fig.cap="시계열 예제", out.extra = ''}
# 데이터 읽기
df_time_series = 
...
```{r time-series, fig.width=4, fig.height=2.5, fig.cap="시계열 예제", out.extra = ''}
# 3개의 시계열 다시 출력
plot_time_series
# MIC 값 출력
mine_ts$MIC
...
```{r Dynamic-Time-warping, echo=FALSE, out.width="300px", fig.cap="동적 시간 워핑 (DTW)", out.extra = ''}
...
```{r correlation-information-theory, tidy=FALSE, fig.cap="상관 관계 플롯", out.extra = ''}
# 행렬을 플롯하는 라이브러리
library(corrplot) 
...
```{r profiling-target-variable, fig.height=2, fig.width=3, fig.cap="cross-plot을 사용한 시각적 분석", out.extra = ''}
...
```{r importance-variable-ranking, fig.width=6, fig.height=4.5, tidy=FALSE, fig.cap="정보 이론을 사용한 상관 관계", out.extra=''}
...
```{r correlation-matrix-information-theory, eval=FALSE, fig.cap="상관 관계 행렬"}
library(infotheo)
# 모든 변수 이산화
...
```{r Comparing-MIC-and-mutual-information-index, echo=FALSE, out.width="100%", fig.cap="변수 중요도 비교"}
