...
최소값과 최대값을 함수를 사용하여 할당하는 것이 좋은 관행입니다. 이 사례에서 변수는 백분율이므로 범위가 0에서 100 사이임을 미리 알고 있지만, `r emo::ji("warning")` _만약 범위를 모른다면 어떤 일이 벌어질까요?_
...
**모든 구간이 바뀌었습니다!** `r emo::ji("scream")` 이들은 새로운 카테고리이기 때문에, 예측 모델은 이들을 모두 새로운 값으로 인식하여 제대로 처리하지 못할 것입니다.
...
#### 보너스 트랙: 트레이드오프의 예술 `r emo::ji("balance_scale")`
...
### 사례 `r emo::ji("mag")` {#analyze-binary-outcome-vs-categorical-variable}
...
### 예측 모델링을 위한 분석 `r emo::ji("crystal_ball")`
...
**개선도: ~ `r round(100*(new_roc-roc)/roc,2)`%**. `r emo::ji("white_check_mark")`
...
### 수치형 또는 다중 공선성 타겟 변수 `r emo::ji("straight_ruler")`
...
### 그룹화에서 얻은 "추가 선물 `r emo::ji("gift")`"은 무엇인가요?
...
변수가 왼쪽으로 치우쳐 있고, 오른쪽에 일부 이상치 점들이 보입니다. 우리는 이들을 _처리_하고 싶습니다 (`r emo::ji("sunglasses")`). 그렇다면 질문이 생깁니다: _극단의 임계값을 어디에 설정해야 할까요?_ 직관적으로는 상위 1%로 설정하거나, 상위 1%를 제거한 후 평균의 변화를 분석할 수 있습니다.
...
`r emo::ji("japanese_goblin")`: _"오! 하지만 그건 서로 다른 두 위치를 사용한 극단적인 예시잖아요!"_
...
`r emo::ji("thinking")`: _"좋아요. 하지만 이런 개념들이 머신러닝과 무슨 상관이 있죠?"_
...
`r emo::ji("pushpin")` 원래의 문제(_추운 날씨는 어디서 끝나는가?_)로 돌아가서, 모든 질문에 답이 필요한 것은 아닙니다. 어떤 질문들은 단순히 우리가 생각하는 데 도움을 줄 뿐입니다.
...
#### 결과 전달 (Communicating results) `r emo::ji("earth_africa")` `r emo::ji("mega")`
...
* **수치형 (Numerical)** `r emo::ji("straight_ruler")`: 앞서 본 것과 같습니다.
...
* **범주형 (Categorical)** `r emo::ji("bar_chart")`: 카테고리의 분산이 매우 높은(고카디널리티) 변수를 갖는 경우입니다: 예: 우편번호. 범주형 변수의 이상치를 처리하는 방법에 대한 자세한 내용은 [기술 통계에서의 고카디널리티 변수](#high_cardinality_descriptive_stats) 장을 참조하세요.
...
오 `r emo::ji("scream")`! `Argentina`와 `cat_B`의 조합은 다른 값들(1% 미만)에 비해 _매우 낮습니다_(0.75%). 반면 다른 교차점들은 22% 이상입니다.
...
### 1단계: 이상치를 감지하는 방법 `r emo::ji("mag_right")`
...
### 2단계: 이상치를 어떻게 할까요? `r emo::ji("hammer_and_wrench")`

우리는 이미 어떤 점들이 이상치인지 감지했습니다. 따라서 이제 질문은 다음과 같습니다: _이들을 어떻게 할까요?_ `r emo::ji("thinking")`
...
_숫자를 보고 변수 분포를 시각화하는 이 마지막 작업은 다른 사람이 해주는 말을 듣고 그림을 상상하는 것과 같습니다. 우리는 목소리(신호)를 우리 뇌 속의 이미지로 변환합니다._ `r emo::ji("speaking_head")` `r emo::ji("roll_eyes")` ... => `r emo::ji("mountain_snow")`
...
독자가 직접 해볼 수 있는 **좋은 테스트**는 데이터셋을 가져와서 이상치를 처리한 다음 Kappa, ROC, 정확도 등과 같은 몇 가지 성능 지표를 비교해 보는 것입니다. **데이터 준비가 이 중 하나라도 개선했나요?** 또는 보고 시에 평균이 얼마나 변했는지 확인해 보세요. 어떤 변수를 플롯팅했을 때 이제 그 플롯이 우리에게 무언가를 말해 주나요? 이런 방식으로 독자는 자신의 경험을 바탕으로 새로운 지식을 창조하게 될 것입니다 `r emo::ji("wink")`.
...
이들은 R에서 실용적인 접근 방식을 사용하여 예시될 것입니다. 코드는 여러분의 프로젝트에 적용할 수 있을 만큼 범용적일 것입니다 `r emo::ji("slightly_smiling_face")`.
...
한 가지 중요한 질문이 생깁니다: _어떻게 해야 할까요?!_ `r emo::ji("scream")`
...
library(dplyr) 
library(funModeling) 
df_status(heart_disease, print_results = F) %>% select(variable, q_na, p_na) %>% arrange(desc(q_na))
```
...
```{r}
# NA 값을 가진 변수 이름 가져오기
vars_to_exclude = df_status(heart_disease, print_results = F) %>% 
  filter(p_na > 0) %>% 
  pull(variable)
...
보시다시피, 모든 개념들 사이에는 상호 관계가 있습니다 `r emo::ji("wink")`.
...
**조언:** 결측값 대치에 대한 초기 접근 방식으로는 이 방법이 정말 복잡합니다 `r emo::ji("fearful")`.
...
_여전히 0이 아니네요! 시간이 얼마나 더 필요한 거죠?!_ `r emo::ji("scream")`
...
하지만 이것은 _"실생활"_에서도 일어납니다. 모든 연령대에 적합한 데이터 과학 책을 쓸 때, 그것을 끝내는 데 얼마나 많은 시간이 필요할까요? 무한한 시간일까요? 아마도 아닐 겁니다 `r emo::ji("smile")`.
