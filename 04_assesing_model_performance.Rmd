...
#### 절단점(Cut point) 사용하기 `r emo::ji("scissors")`
...
이벤트는 일어날 수도 있고, 일어나지 않을 수도 있습니다. 비록 우리에게 *내일의 신문* `r emo::ji("newspaper")`은 없지만, 내일이 어떨지에 대해 좋은 추측을 할 수는 있습니다.
...
`r emo::ji("raised_hand")` **R 구문** -_코드를 보고 싶지 않다면 건너뛰셔도 됩니다_-
...
### 모든 것은 절단점(cut point)에 달려 있습니다 `r emo::ji("straight_ruler")`
...
* `r emo::ji("white_check_mark")` **진양성 (True Positive, TP)**: 분류가 *양성*인 것이 *참*인 경우, 즉 "모델이 양성(`예`) 클래스를 정확하게 맞춤"을 의미합니다.
* `r emo::ji("white_check_mark")` **진음성 (True Negative, TN)**: 위와 동일하지만 음성 클래스(`아니오`)의 경우입니다.
* `r emo::ji("x")` **가양성 (False Positive, FP)**: 분류가 *양성*인 것이 *거짓*인 경우, 즉 "모델이 틀렸고 `예`라고 예측했지만 결과는 `아니오`임"을 의미합니다.
* `r emo::ji("x")` **가음성 (False Negative, FN)**: 위와 동일하지만 음성 클래스의 경우로, "모델이 음성이라고 예측했지만 결과는 양성이었음", 즉 "모델이 `아니오`라고 예측했지만 클래스는 `예`였음"을 의미합니다.
...
`r emo::ji("thumbsup")` 최선의 시나리오는 **TP**와 **TN** 비율이 100%일 때입니다. 이는 모델이 모든 `예`와 모든 `아니오`를 정확하게 예측했음을 의미합니다. (결과적으로 **FP**와 **FN** 비율은 0%가 됩니다).

하지만 잠깐만요 `r emo::ji("raised_hand")`! 완벽한 분류를 발견했다면, 그것은 아마도 과적합 때문일 것입니다!

`r emo::ji("thumbsdown")` 최악의 시나리오 —이전 예시와 정반대— 는 **FP**와 **FN** 비율이 100%일 때입니다. 무작위성조차도 이토록 끔찍한 시나리오를 만들어낼 수는 없습니다.
...
`r emo::ji("question")` **퀴즈**: 이 33.3%를 정확하게 예측하는 모델(TP 비율=100%)은 좋은 모델인가요?
